...
//          x_fill.rgb = vec3(luma);
        ...
        val circleRadius = 40.0

        val circleParameters1 = List(36) {
...
//                    canopy1SqBlur.colorBuffer(0).copyTo(blurred.colorBuffer(0))
        ...
        extend {
            ...
            drawer.isolatedWithTarget(movingCircles1) {
                ...
                leaf.parameter("rotation", seconds)
                drawer.shadeStyle = leaf
                for (params in circleParameters1) {
                    drawer.circle(canopyWidth*(params[0] +0.1*cos(seconds*params[2]/2.0 + params[3])),
                        canopyHeight*(params[1] + 0.1*sin(seconds*params[3]/2.0 + params[2])),
                        circleRadius)
                }
            }
            ...
            drawer.isolatedWithTarget(movingCircles2) {
                ...
                leaf.parameter("segmentFactor", 0.1)
                leaf.parameter("smoothing", 0.8)
                leaf.parameter("rotation", seconds/2)
                drawer.shadeStyle = leaf
                for (params in circleParameters2) {
                    drawer.circle(canopyWidth*(params[0] +0.2*cos(seconds*params[2]/2.0 + params[3])),
                        canopyHeight*(params[1] + 0.2*sin(seconds*params[3]/2.0 + params[2])),
                        circleRadius)
                }
